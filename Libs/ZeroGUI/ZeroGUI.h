...
#include "./ZeroInput.h"
#include "../Config.h"
#include "./Ext/skCrypter.h"
using namespace SDK;
wchar_t* s2wc(const char* c)
{
	// 使用CP_ACP代替CP_UTF8来处理BOM
	int size_needed = MultiByteToWideChar(CP_ACP, 0, c, -1, NULL, 0);
	wchar_t* wc = new wchar_t[size_needed];
	
	// 执行转换
	MultiByteToWideChar(CP_ACP, 0, c, -1, wc, size_needed);
...
namespace ZeroGUI
{
	...
	namespace PostRenderer
	{
		...
		void Draw_Line(FVector2D from, FVector2D to, float thickness, FLinearColor color)
		{
			for (int i = 0; i < 128; i++)
			{
				if (drawlist[i].type == -1)
	...
	void TextLeft(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		canvas->K2_DrawText((UFont*)Config::GetInstance().font, FString{ s2wc(name) }, pos, FVector2D{ 1.0f, 1.0f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}	
	void TextCenter(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		canvas->K2_DrawText((UFont*)Config::GetInstance().font, FString{ s2wc(name) }, pos, FVector2D{ 1.0f, 1.0f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	void SliderInt(char* name, int* value, int min, int max)
	{
		...
		if (isHovered || current_element == elements_count)
		{
			...
			if (Input::IsMouseClicked(0, elements_count, true))
			{
				...
				FVector2D cursorPos = CursorPos();
				*value = (int)(((cursorPos.X - pos.X) * ((max - min) / slider_size.X)) + min);
	...
	std::string VirtualKeyCodeToString(UCHAR virtualKey)
	{
		...
		{
			return std::string(skCrypt("MOUSE0"));
		}
		if (virtualKey == VK_RBUTTON)
		{
			return std::string(skCrypt("MOUSE1"));
		}
		if (virtualKey == VK_MBUTTON)
		{
			return std::string(skCrypt("MBUTTON"));
		}
		if (virtualKey == VK_XBUTTON1)
		{
			return std::string(skCrypt("XBUTTON1"));
		}
		if (virtualKey == VK_XBUTTON2)
		{
			return std::string(skCrypt("XBUTTON2"));
		}
	...
	void Hotkey(char* name, FVector2D size, int* key)
	{
		...
		if (active_hotkey == elements_count)
		{
			//Text
			FVector2D textPos = FVector2D{ pos.X + size.X / 2, pos.Y + size.Y / 2 };
			TextCenter(skCrypt("[Press Key]"), textPos, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);
